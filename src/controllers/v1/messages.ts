...
export const createMessage = async (request: Request, response: Response) => {
  ...
  try {
    const query = `
      INSERT INTO messages (content, subject, author_id)
      VALUES ($1, $2, $3)
...
export const updateMessage = async (request: Request, response: Response) => {
  ...
  try {
    const query = `
      UPDATE messages
      SET content = $1, subject = $2, author_id = $3, updated_at = NOW()
      WHERE id = $4
      RETURNING id, content, subject, read, author_id AS "authorId",
                created_at AS "createdAt", updated_at AS "updatedAt"
    `;
    const result = await pool.query(query, [content, subject || null, authorId, id]);
...
export const patchMessage = async (request: Request, response: Response) => {
  ...
  for (const field of validFields) {
    if (request.body[field] !== undefined) {
      const column = field === 'read' ? 'read' : field;
